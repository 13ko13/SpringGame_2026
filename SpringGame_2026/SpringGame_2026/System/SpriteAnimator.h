...
public:
	...
	void Init(int drawHandle, int animIndex, Vector3 frameSize, int maxAnimNum, int oneAnimFrame, float scale, bool isRepeat);
	/// <summary>
	/// 普通のアニメーション初期化
	/// </summary>
	/// <param name="drawHandle">画像ハンドル</param>
	...
	/// <param name="scale">表示サイズ</param>
	/// <param name="isRepeat">ループするかどうか</param>
	void Init(int drawHandle, int startSrcX, int animIndexY, Vector3 frameSize, int maxAnimNum, int oneAnimFrame, float scale, bool isRepeat);
	/// <summary>
	/// 一枚絵アニメーション初期化
	/// </summary>
	/// <param name="drawHandle">画像ハンドル</param>
	...
	/// <param name="scale">表示サイズ</param>
	/// <param name="isRepeat">ループするかどうか</param>
	void Init(int drawHandle, int animIndexY, int animIndexX, Vector3 frameSize, float scale, bool isRepeat = true);
	void Update();
	void Draw(Vector3 pos, bool isTurn);
	void Draw(int drawHandle, Vector3 pos, bool isTurn);

	void SetRotate(float rotate) { m_rotate = rotate; }
	void SetOffset(Vector3 offset) { m_drawOffset = offset; }
	// アニメーションを最初から再生する
...
private:
	...
	float m_scale;		// 拡大率
	float m_rotate;		// 回転角
	Vector3 m_drawOffset; // 描画位置オフセット
};